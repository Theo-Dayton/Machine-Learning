...
def calc_binary_metrics(y_true_N, y_hat_N):
    ...
    """
    # TODO
    TP = sum((y_true_N == 1) & (y_hat_N == 1))
    TN = sum((y_true_N == 0) & (y_hat_N == 0))
    FP = sum((y_true_N == 0) & (y_hat_N == 1))
    FN = sum((y_true_N == 1) & (y_hat_N == 0))
...
def calc_percent_cancer(labels, cancer_label=1):
    ...
    num_cancer = sum(labels == cancer_label)
    percent_cancer = num_cancer / len(labels) * 100
...
def calc_perf_metrics_for_threshold(y_true_N, y_proba1_N, thresh=0.5):
    ...
    TP = ((y_pred_N == 1) & (y_true_N == 1)).sum()
    TN = ((y_pred_N == 0) & (y_true_N == 0)).sum()
    FP = ((y_pred_N == 1) & (y_true_N == 0)).sum()
    FN = ((y_pred_N == 0) & (y_true_N == 1)).sum()
...
def calibrated_perceptron_classifier(x_train, y_train, x_test, y_test,
    ...
    cccv = CalibratedClassifierCV(model, method='isotonic')
    cccv.fit(x_train, y_train)

    pred_train = cccv.predict_proba(x_train)[:, 1]
    pred_test = cccv.predict_proba(x_test)[:, 1]
