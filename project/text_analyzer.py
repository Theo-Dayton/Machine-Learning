...
def get_top_k(kv_dict: Dict[str, float], k: int = 20) -> List[Tuple[str, float]]:
    ...
    top_k = []  # TODO: fix me
    sorted_list = sorted(kv_dict.items(), key=lambda item: item[1])
    num_pairs = k
    for i in range(num_pairs):
        top_k.append(sorted_list.pop())
    return top_k
...
def sort_dictionary_by_value(
    ...
    if direction == "ascending":
        sort_dict = sorted(dict_in.items(), key=lambda item: item[1])
    elif direction == "descending":
        sort_dict = sorted(dict_in.items(), key=lambda item: item[1], reverse=True)
    return sort_dict
...
def tf(document: List[str]) -> Dict[str, int]:
    ...
    unique_terms = list(set(document))
    for term in unique_terms:
        document_tf.update({term.lower(): 0})
    for term in document:
        document_tf[term.lower()] += 1
...
def idf(corpus: Dict[str, List[str]]) -> Dict[str, float]:
    ...
    for doc in corpus:
        total_words = total_words + corpus[doc]
    ...
    for word in unique_words:
        ...
        for doc in corpus:
            if word in corpus[doc]:
                count += 1
        score = math.log(np.divide(float(N), float(count)))

        corpus_idf.update({word: score})
...
def tf_idf(
    ...
    for term in sonnet_tf:
        if term in corpus_idf:
            corpus_tf_idf.update({term : corpus_idf[term] * sonnet_tf[term]})
        else:
            corpus_tf_idf.update({term: 0})
...
def cosine_sim(
    ...
    unique_words = list(set({**vec1, **vec2}))
    numerator = 0
    sum1 = 0
    sum2 = 0
    for word in unique_words:
        if word in list(vec1) and word in list(vec2):
            numerator += vec1[word]*vec2[word]
    for word in list(vec1):
        sum1 += np.square(vec1[word])
    for word in list(vec2):
        sum2 += np.square(vec2[word])
    similarity = np.divide(numerator, np.sqrt(sum1*sum2))
